...
Function OnGameLoad()
	...
	if F4SE.GetPluginVersion("GardenOfEdenPapyrusScriptExtender") == -1 || GardenOfEden.GetVersionRelease() < 220
		Var[] args = new Var[1]
...
Function TogglePipBoyLight()
	...
	scanCodeKBM = Input.GetMappedKey("Pipboy", 0)
	if scanCodeKBM == -1
		scanCodeKBM = Input.GetMappedKey("Pipboy", 1)
	endif
	if scanCode != scanCodeKBM && scanCodeKBM != -1
		RegisterForKey(scanCodeKBM)
	endif
